...
- **Draft data is 2025-vintage, used as a 2026 proxy.** The market pool is the **DraftSheets `8.13.25` export (2025 NFL season, imported `season=2025`)** and canonical players are nflverse-through-2025. So the draftable "rookies" are the **2025 NFL draft class** (Jeanty, Hampton, Judkins, Egbuka…) — who are 2nd-year players as of the app's date (2026-05-28). **The 2026 rookie class — drafted in April 2026 (e.g. Jeremiyah Love, Tate) — is NOT in the pool**: they weren't NFL players in 2025, so they're absent from the Aug-2025 source (confirmed: not in the workbook DATA/ECR sheets). The data is effectively **one year stale** for a 2026 draft. This is the #1 "why isn't player X here?" cause — check the source workbook before treating a missing player as a bug.

  **2026 data path (decided direction — Rob has no 2026 DraftSheets file, so we go source-agnostic):** the market layer is already source-agnostic (`DraftPlayerMarket` keys on `(source, season, canonical_player_id)`), so DraftSheets is just *one* ranking source. Split the need: **(a) Identity** — the 2026 player universe (incl. rookies) is largely solvable now via **Sleeper** (`/v1/players/nfl` already carries 2026 rookies, and `refresh_sleeper_context` already creates canonical rows for draftables). **(b) Rankings** — the real gap: a **2026 ADP/ECR source**. DraftSheets was only a packaging of ECR. Decision (Rob): ingest **both** — they're complementary signals, and the market table already holds multiple sources side by side. **FantasyPros ECR** (expert consensus) + **Sleeper ADP** (where the market actually drafts). **Merge strategy for the board:** ECR drives `effective_rank` (ordering), Sleeper ADP fills the `adp` column, `value_delta = ADP − ECR` — mirrors what DraftSheets did, now from agnostic 2026-current sources, and properly powers the value/faller logic. **Codex (data lane):** build both ingesters writing `DraftPlayerMarket(source=fantasypros|sleeper_adp, season=2026)` + the merge into a single board row per canonical player; suggest incremental (one source end-to-end first to prove the pipeline, then add the second + merge). **Claude verifies** the 2026 class lands with sane ECR/ADP/value afterward. Until then the board is 2025-vintage.
