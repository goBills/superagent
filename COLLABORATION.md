...
- **🔴 ACTIVE (P0) BUG — agent recommends already-drafted players (season mismatch).** Live mock, pick 59: "who do I take next" returned Bijan Robinson (rank 2.2) and Saquon Barkley (3.7) as top available — both drafted in round 1. **Root cause (Claude diagnosed):** `tool_schemas.py` describes `season` as "NFL season year (**2020-2025**)" on every tool, so the agent passes `season=2025` for draft questions. But the live board + recorded picks are **2026** (`sleeper_adp`). Result: (1) the agent reads the **stale 2025 DraftSheets board** (the "Effective Rank 2.2 (avg rank)" tell — avg_rank is a 2025 column; 2026 uses ADP), and (2) `_drafted_ids(league, season=2025)` finds **none** of the 2026-recorded picks → no exclusion → drafted elites resurface. The *board* (`get_draft_sheet`) is correct because it runs on 2026. **Fix (Codex's agent/season layer):** make draft-market tools operate on the current board season — simplest: for the DRAFT tools (find_draft_targets, get_available_targets, get_draft_sheet, get_draft_context, get_position_needs, get_roster_construction_context, recommend_next_pick_targets, compare_draft_options, get_bye_week_analysis) change the `season` schema to "draft market season; OMIT to use the current board (recommended)" and/or allow 2026, so the agent leaves it unset and the existing `season or _latest_market_season(db)` (→2026) default applies; keep STAT tools (player/team summaries) at 2020-2025. Update the agent prompt to never pick a draft season. **Claude can take the contained schema-description edit if you want — flag it; otherwise it's yours since it overlaps the 2026/agent build.** Claude verifies post-fix that mid-draft "who's next" only returns available players.
