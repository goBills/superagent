...
- **P0 — K/DST on the 2026 board. DONE / LIVE VERIFIED.** Carried the stable 2025 K/DST rows onto the 2026 board (shipped by Codex, **v0.14.1** "Carry K and DST into Sleeper ADP board"). *(Claude verified live `426ae31`: `position=K`→12, `position=DST`→12 on `season=2026`; within the draftable 192 they sit in the late-round window — K eff_rk ~169–182, DST ~170–181 — pool still 192/`pool_shortfall:0`.)* So the 2026 board is now a complete 16-round draft (skill + K + DST) and the K/DST tabs populate. **Minor note (not blocking):** the All-192 view surfaces only the top ~6 K / ~8 DST (rank cap); the K/DST *tabs* show all 12 each, which is how you draft them late — could reserve a few more K/DST slots in the All pool later, but tabs cover it.
