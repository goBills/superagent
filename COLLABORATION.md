...
- **Draft data is 2025-vintage, used as a 2026 proxy.** The market pool is the **DraftSheets `8.13.25` export (2025 NFL season, imported `season=2025`)** and canonical players are nflverse-through-2025. So the draftable "rookies" are the **2025 NFL draft class** (Jeanty, Hampton, Judkins, Egbuka…). **True 2026-draft-class prospects (still in college in 2025 — e.g. Jeremiyah Love) are NOT in the pool**, and never will be until we import a **2026 DraftSheets export + a 2026 canonical universe** (available later in 2026, post-draft). This is the #1 "why isn't player X here?" cause — verify against the source workbook (`~/Downloads/Copy of DraftSheets Fantasy Tool.xlsx`, DATA/ECR sheets) before treating a missing player as a bug. The "🏈 2026 Draft" UI label overstates the vintage; consider being explicit that rankings are a 2025 proxy.
