...
- **P0 — Pool depth. CODE FIX SHIPPED, DATA REFRESH NEEDED.** v0.13.4 (`c3fc4e5`) maps DraftSheets `DST`/`D/ST`/`DEF` rows to stable `team_dst_*` canonical IDs, sets their real NFL team code for byes, prevents false player collisions (e.g. Dallas Cowboys ≠ Dallas Turner), and marks old pending review rows resolved when a clean re-import maps them. **Operational next step:** after v0.13.4 deploys, re-run the DraftSheets import so persisted prod market rows pick up the missing team defenses; then re-check 12×16 `available_count` vs `total_draft_picks`.
