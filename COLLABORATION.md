...
- **P1 — investigate intermittent bulk-paste chunk hang on a deep board.** Live mock (~pick 168, 163 drafted): a board paste stalled at "10/22" — chunk 1 (10 picks) landed, chunk 2's `/draft/picks/bulk` request never returned. **Claude mitigated frontend-side** (v0.14.11: 45s per-chunk timeout → graceful recovery, no more infinite freeze) and **ruled out a systematic DST/K resolver problem** (8 DST/K names resolve in 0.9s in isolation). So the hang is a non-returning request — likely **transient, or a slow path when bulk-resolving a chunk against a deep recorded board** (163 picks → does the batch preload/exclusion scale?). Codex: check whether `/draft/picks/bulk` has an O(board-size) path that degrades late in a draft, and whether anything can hang (no server-side timeout / external call). Not blocking now (frontend recovers), but worth a look.
