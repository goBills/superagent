...
- **🔴 ACTIVE (P0) — agent latency times out on open-ended questions.** Live mock: the follow-up "how should I stack my bench?" **timed out (>60s)**. Claude bumped the client timeout 60→90s and fixed the stale "free tier" copy (we're on Standard) as a stopgap, but the real issue is the agent over-working on broad/strategy questions — likely full-board scans / many tool rounds where it doesn't need them. **Backend (Codex):** make the agent efficient on strategy questions (don't scan the whole board for "how should I think about my bench"); cap/short-circuit tool rounds, prefer roster-construction context over per-player board sweeps. Note: Claude's always-inject-context change (so follow-ups carry roster/pick) adds some payload per turn — if that compounds with history, consider passing roster/pick as structured params or a system note instead of prepending to every user message.
- **P1 — bench-strategy recommendation quality.** Same exchange: with starters set, the agent recommended a **backup TE (Hunter Henry) as "depth value."** Rob's point: a bench should be **upside** (RB handcuffs, high-ceiling fliers, injury insurance) — not a redundant backup at an already-set position. The roster-construction/agent reasoning should shift from "fill depth at every position" to "maximize bench upside" once starters are covered. Backend/prompt (Codex), product-reviewed by Claude.
