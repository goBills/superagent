...
- **P0 — Pool depth. CODE + DATA REFRESH SHIPPED; DEPLOY VERIFY v0.13.6.** v0.13.4 (`c3fc4e5`) maps DraftSheets `DST`/`D/ST`/`DEF` rows to stable `team_dst_*` canonical IDs, sets their real NFL team code for byes, prevents false player collisions (e.g. Dallas Cowboys ≠ Dallas Turner), and marks old pending review rows resolved when a clean re-import maps them. v0.13.5 (`7b039ce`) added safe `replace=true` imports. **Prod import run by Codex 2026-05-28:** `source=draftsheetsv6`, `season=2025`, `sheet=DATA`, `replace=true`, `min_replace_rows=700`; deleted 812 stale markets / 476 pending reviews and imported 844 mapped rows from 966 workbook rows. D/ST is no longer stuck in review. First smoke showed 12×16 improved 178→187 but still short 5 because DraftSheets ADP is sparse/gapped around pick 192; v0.13.6 (`d17108a`) backfills the all-position sheet with the next best ranked rows until remaining draft slots are covered. **Operational next step:** once v0.13.6 deploys, re-run the same guest smoke: default 12×16 should show `available_count:192`, `remaining_picks:192`, `pool_shortfall:0`.
