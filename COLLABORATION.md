...
- **P1 — Resolver batching (the remaining throughput lever).** Skip-unchanged doesn't help the **first** full-board paste — those picks are all new, so they all hit the resolver. Measured on live: **~6s for 27 clean picks (~0.22s/pick), worse when names miss the alias table and fall to fuzzy** (full-table scan per miss). Note: we're on **Render Standard now (no spin-down)**, so this is warm resolution cost, not a cold-start tax. The fix is to resolve a batch in one pass: load the alias/canonical tables **once**, match all names in-memory, and only fuzzy-fallback the misses (against the in-memory set) — instead of per-name full scans + N+1 queries. Client now chunks large pastes (v0.13.2) for progress/robustness, but that doesn't reduce total resolution work — this does.
...
- **Later — In-season data foundations** (rosters/usage/injuries) to light up the Roster/Waiver "Coming soon" workspaces. *SportsDataIO spike done (Codex, `d99368a`, read-only, 5/5 endpoints).* **Product guardrails before anything wires in:**
