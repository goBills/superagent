...
| **Beat-writer / agent-farm news+injury moat** | **3-way design — SPEC DRAFTED** | The big strategic bet (Rob + Salam both lit up). **Full design spec now in [`docs/news_moat_plan.md`](docs/news_moat_plan.md)** (Claude, 2026-05-30): 4-tier agent farm (collectors → per-team aggregators → synthesis → editor/credibility gate), the `player_news_signal` owned record, a source trust hierarchy + no-fabrication rules mirroring the trade gates, and an honest §5 ToS posture (build the spine on **official injury reports + outlet/beat RSS**; defer **X/social to a gated Phase-3 via official-API-or-licensed only** — do not bet the commercial product on X scraping). **Still design-first — nothing built.** Phasing: Phase-0 = a cheap one-team `Workflow` spike to prove signal-not-fluff quality + cost; Phase-1 = persistence + hover injury notes (Salam's direct ask). **Codex lane (§9):** `player_news_signal` model+migration, scheduler/refresh job, RSS/HTTP fetch infra (`robots.txt` compliance, caching/diff), prose-mention→`canonical_player_id` crosswalk. **Awaiting Rob's 3 decisions in §10** (social posture, greenlight the spike, source seed list) before anyone writes code. |
