...
- **P0 — Pool depth. CODE FIX SHIPPED, DATA REFRESH NEEDED.** v0.13.4 (`c3fc4e5`) maps DraftSheets `DST`/`D/ST`/`DEF` rows to stable `team_dst_*` canonical IDs, sets their real NFL team code for byes, prevents false player collisions (e.g. Dallas Cowboys ≠ Dallas Turner), and marks old pending review rows resolved when a clean re-import maps them. **Operational next step:** after v0.13.4 deploys, re-run the DraftSheets import so persisted prod market rows pick up the missing team defenses; then re-check 12×16 `available_count` vs `total_draft_picks`. *(v0.13.4 live on `cc6efbd`; re-import is Codex's data op — needs the prod `ADMIN_TOKEN`; source file `~/Downloads/Copy of DraftSheets Fantasy Tool.xlsx`, `DATA` tab confirmed. Claude verifies the shortfall once it's run.)*
  - **Recommended follow-up — authoritative `replace=true` import (Codex, endorsed).** Today's import is additive/upsert, so re-importing a newer workbook leaves *stale ghost rows* for players that dropped off — cruft accumulates over repeated ranking updates (which will happen all preseason). `replace=true` makes a new workbook the source of truth. **Architectural guardrails before relying on it:** (1) **scope the delete to `source` (+`season`)** — never the whole `DraftPlayerMarket` table; (2) **transactional** — delete + insert in one transaction so a failed/partial import never leaves an empty board mid-draft; (3) **sanity-gate** — refuse to replace if the parsed workbook yields suspiciously few rows (guard against a broken/empty file nuking the pool). **Sequencing:** replace-mode is small — cleanest is to ship it, then do *one* authoritative import (fixes pool + clean baseline in one shot). But don't let it block the live pool fix — if it slips, do the additive re-import now to unblock testing and switch to replace on the next update.
