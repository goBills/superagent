...
- **Future — proprietary value metric** (replaces a licensed ECR; full rationale in the 2026 data-path note above). Build our own "expected value" rank from data we own — nflverse production/EPA + Sleeper current context — and compare against Sleeper ADP; the divergence is the value/faller signal. Dodges FantasyPros licensing, differentiates the product. Real modeling build (quality bar — a crude metric is worse than ADP-only; 2026 rookies are a cold-start needing draft-capital/college basis). Naming parked. Board is ADP-only until then.
