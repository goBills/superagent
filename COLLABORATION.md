...
- **P0 — Bulk-paste perf.** Re-pasting a growing board reached ~40s late in the draft. *Client half shipped (v0.12.2, `c15b229`):* the frontend now diffs the paste against its local `draftBoardPicks` and POSTs only new/changed picks (identical re-paste makes no call). **Backend half still needed (defense-in-depth):** `/draft/picks/bulk` should skip unchanged existing picks server-side too, so perf doesn't depend on the client — and it's the safety net when a client's local board is stale and over-sends. These stack: frontend sends fewer rows; backend resolves fewer rows even if a client sends the whole board. Add server-side tests for the skip-unchanged invariant.
